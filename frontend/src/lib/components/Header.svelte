      ...
      {#if showHelp}
        <div class="help-panel">
          ...
          <ol class="help-steps">
            ...
            <li><strong>Audio Only page</strong> — Drop audio on its own and choose <strong>AUDIO ONLY</strong> to open a page built for it, in three sections. <strong>QC</strong> measures every file against one spec — LUFS or true peak, optional 6-frame check, and a stereo/phase check that flags dual mono, one-sided and anti-phase files — and lays the results out in a table. <strong>FILE DELIVERABLES</strong> render new files to a spec, on every file, right now: <strong>NORMALISE ALL</strong>, <strong>6 Fr ALL</strong>, <strong>CLOCK ALL</strong> (10 seconds of silence at the head and 5 at the tail) and <strong>CONVERT ALL</strong> to the format, sample rate and bit depth set in the output bar (WAV, AIFF, FLAC, ALAC or AAC; 44.1k / 48k / 96k; 16, 24 or 32-bit float). <strong>FILE PROCESSING</strong> changes the file's shape: split a multichannel file into named mono stems (_L, _R, _C, _LFE, _Ls, _Rs), join a mono set back into a stereo, 5.1 or 7.1 file, fold surround to stereo or anything to mono, fade the ends, or trim head-and-tail silence. New files are always written beside the originals and the list reloads with them, re-measured. <strong>← LAYBACK</strong> takes you back to the layback page.</li>
